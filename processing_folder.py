...
class PSDLayerExtractor:
    ...
    def _manual_composite_preview(self):
        ...
        final.save(preview_path, 'PNG', quality=95)
    ...
    def extract(self):
        ...
        try:
            ...
            # print(f"输出文件夹: {self.file_output_folder}")
            # print("-" * 50)
            ...
            # print(f"所有文件已保存到: {self.file_output_folder}")
            return True
        except Exception as e:
...
def main():
    # 使用示例 - 使用原始字符串避免转义问题
    psd_folder = r"/storage/human_psd/psd_fp_v1"  # 包含PSD文件的文件夹路径
    output_folder = r"/storage/human_psd/fp_v1_output"  # 输出文件夹
    ...
    # 使用tqdm显示进度
    for psd_file in tqdm(psd_files, desc="处理PSD文件"):
        try:
            # 创建提取器并执行
